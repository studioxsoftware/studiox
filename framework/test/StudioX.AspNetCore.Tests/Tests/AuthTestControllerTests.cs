...
{
    public class AuthTestControllerTests : AppTestBase
    {
        [Fact]
        public async Task CanCallNonAuthorizedActionsAnonymously()
        {
            // Act
        ...
        [Fact]
        public async Task CanNotCallAuthorizedActionsAnonymously()
        {
            // Act & Assert
            ...
            await Assert.ThrowsAnyAsync<Exception>(async () =>
            {
                await GetResponseAsStringAsync(
                    GetUrl<AuthTestController>(
                        nameof(AuthTestController.StudioXMvcAuthorizedAction)
                    )
                );
            });
            ...
            var response = await GetResponseAsObjectAsync<AjaxResponse>(
                    GetUrl<AuthTestController>(
                        nameof(AuthTestController.StudioXMvcAuthorizedActionReturnsObject)
                    ),
