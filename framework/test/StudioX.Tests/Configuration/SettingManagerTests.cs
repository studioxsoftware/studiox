...
{
    public class SettingManagerTests : TestBaseWithLocalIocManager
    {
        ...
        private SettingManager CreateSettingManager()
        {
            return new SettingManager(
                ...
                new StudioXMemoryCacheManager(
                    LocalIocManager,
                    new CachingConfiguration(Substitute.For<IStudioXStartupConfiguration>())
                    )
                );
        }

        [Fact]
        public async Task ShouldGetDefaultValuesWithNoStoreAndNoSession()
        {
        ...
        [Fact]
        public async Task ShouldGetStoredApplicationValueWithNoSession()
        {
            var settingManager = CreateSettingManager();
        ...
        [Fact]
        public async Task ShouldGetCorrectValues()
        {
            var session = CreateTestStudioXSession();

            var settingManager = CreateSettingManager();
            settingManager.SettingStore = new MemorySettingStore();
            settingManager.StudioXSession = session;
        ...
        [Fact]
        public async Task ShouldGetAllValues()
        {
            var settingManager = CreateSettingManager();
            settingManager.SettingStore = new MemorySettingStore();
        ...
        [Fact]
        public async Task ShouldChangeSettingValues()
        {
            var session = CreateTestStudioXSession();
        ...
        [Fact]
        public async Task ShouldDeleteSettingValuesOnDefaultValue()
        {
            var session = CreateTestStudioXSession();
        ...
        {
            private readonly List<SettingInfo> settings;
            ...
            {
                return Task.FromResult(settings.FirstOrDefault(s => s.TenantId == tenantId && s.UserId == userId && s.Name == name));
            }
            ...
            {
                return Task.FromResult(settings.Where(s => s.TenantId == tenantId && s.UserId == userId).ToList());
            }
        }
    }
}