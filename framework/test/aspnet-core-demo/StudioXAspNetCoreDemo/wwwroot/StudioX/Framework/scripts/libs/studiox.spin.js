...
(function () {
    ...
    studiox.ui.setBusy = function (elm, optionsOrPromise) {
        ...
        if (!elm) {
            ...
            var $elm = $(elm);
            var $busyIndicator = $elm.find('.studiox-busy-indicator'); //TODO@Long: What if  more than one element. What if there are nested elements?
    ...
    studiox.ui.clearBusy = function (elm) {
        //TODO@Long: Maybe better to do not call unblock if it's not blocked by setBusy
        if (!elm) {
            studiox.ui.unblock();
            $('body').spin(false);
        } else {
