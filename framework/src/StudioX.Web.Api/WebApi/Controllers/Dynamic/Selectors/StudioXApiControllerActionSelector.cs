...
namespace StudioX.WebApi.Controllers.Dynamic.Selectors
{
    ...
    public class StudioXApiControllerActionSelector : ApiControllerActionSelector
    {
        ...
        public override HttpActionDescriptor SelectAction(HttpControllerContext controllerContext)
        {
            ...
            if (!controllerContext.ControllerDescriptor.Properties.TryGetValue("__StudioXDynamicApiControllerInfo", out controllerInfoObj))
            {
                return GetDefaultActionDescriptor(controllerContext);
            }

            //Get controller information which is selected by StudioXHttpControllerSelector.
            var controllerInfo = controllerInfoObj as DynamicApiControllerInfo;
            if (controllerInfo == null)
            {
                throw new StudioXException("__StudioXDynamicApiControllerInfo in ControllerDescriptor.Properties is not a " + typeof(DynamicApiControllerInfo).FullName + " class.");
            }

            //No action name case
            var hasActionName = (bool)controllerContext.ControllerDescriptor.Properties["__StudioXDynamicApiHasActionName"];
            if (!hasActionName)
            {
        ...
        private HttpActionDescriptor GetActionDescriptorByActionName(HttpControllerContext controllerContext, DynamicApiControllerInfo controllerInfo, string actionName)
        {
            ...
            {
                throw new StudioXException("There is no action " + actionName + " defined for api controller " + controllerInfo.ServiceName);
            }
